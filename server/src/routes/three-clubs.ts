...
function verifiedPlayer(clubA: string, clubB: string, clubC: string, id: number) {
  const variants = [clubA, clubB, clubC].flatMap(cl => getClubVariants(cl).map(v => v.toLowerCase()))
...
threeClubsRouter.post(
  ...
  async (c) => {
    ...
    const targetClubs = [clubA, clubB, clubC]
    ...
    if (footballer) {
      ...
      if (playedAll(stintClubs)) {
        return c.json({ valid: true, footballer: verifiedPlayer(clubA, clubB, clubC, footballer.id), imported: false })
      }
      ...
      if (footballer.wikipedia_url) {
        try {
          ...
          stintClubs = (await db.select({ club: career_stints.club })
            ...
            .where(sql`${career_stints.footballer_id} = ${footballer.id} AND ${career_stints.stint_type} = 'senior'`)).map(s => s.club)
          if (playedAll(stintClubs)) {
            return c.json({ valid: true, footballer: verifiedPlayer(clubA, clubB, clubC, footballer.id), imported: true })
          }
        } catch {
    ...
    try {
      ...
      if (byUrl) {
        ...
        if (playedAll(stints.map(s => s.club))) {
          return c.json({ valid: true, footballer: verifiedPlayer(clubA, clubB, clubC, byUrl.id), imported: false })
        }
      }
      ...
      if (knownRecord) {
        ...
        return c.json({ valid: true, footballer: verifiedPlayer(clubA, clubB, clubC, knownRecord.id), imported: true })
      }
      ...
      return c.json({ valid: true, footballer: verifiedPlayer(clubA, clubB, clubC, newFootballer.id), imported: true })
    } catch {
...
threeClubsRouter.get('/answers', async (c) => {
  ...
  `).all(...allVariants, ...variantsA, ...variantsB, ...variantsC) as { id: number; name: string; photo_url: string | null; nationality: string | null; position: string | null; apps: number; career_start: string | null; career_end: string | null }[]
