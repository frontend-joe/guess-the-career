...
import { getWsmSession, submitWsmScore, getWsmLeaderboard } from '@/api/who-scored-more'
    ...
    <div className="h-dvh flex flex-col w-full max-w-[400px] mx-auto font-sans">
      ...
      <div className="bg-[#1a1a2e] flex items-center justify-between px-3 py-2 shrink-0">
        ...
        <div className="flex items-center gap-2">
          {players.length > 0 && status !== 'won' && status !== 'lobby' && view === 'game' && (
          ...
          <button
            ...
            onClick={() => setView(v => v === 'leaderboard' ? 'game' : 'leaderboard')}
          >
            <Trophy size={20} />
          </button>
        </div>
            ...
            {!loading && !error && status === 'lobby' && (
              <div className="flex-1 flex flex-col items-center justify-center px-6 gap-6 text-center">
                <div>
                  ...
                  <h2 className="text-xl font-bold text-gray-800">Who Scored More?</h2>
                  <p className="text-sm text-gray-500 mt-2">10 rounds · Timer starts when you're ready</p>
                </div>
                <button
                  onClick={handleStart}
      ...
      {/* Bottom panel */}
      {view === 'game' && !loading && !error && status !== 'won' && status !== 'lobby' && (
  ...
  return (
    <div className="flex flex-col items-center px-4 py-8 gap-5">
      <div className="text-center">
        ...
        <p className="text-lg font-mono font-bold text-gray-500 mt-2">⏱ {formatTime(elapsedMs)}</p>
      </div>
...
function LeaderboardView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col flex-1 px-4 py-6">
      ...
      {error && (
        <p className="text-red-500 text-sm text-center mt-8">Failed to load leaderboard.</p>
      )}

      {!loading && !error && entries.length === 0 && (
        <p className="text-gray-400 text-sm text-center mt-8">No scores yet — be the first!</p>
      )}
          ...
          {entries.map((entry, i) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <span className={`text-sm font-bold shrink-0 w-6 text-center ${i === 0 ? 'text-yellow-500' : i === 1 ? 'text-gray-400' : i === 2 ? 'text-amber-600' : 'text-gray-300'}`}>
                  {i + 1}
                </span>
                <span className="text-sm font-semibold text-gray-800 truncate">{entry.player_name}</span>
              </div>
