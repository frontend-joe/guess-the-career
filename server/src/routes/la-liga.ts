...
export const laLigaRouter = new Hono()

const MIN_PLAYERS = 3


// The Spanish club a player made the most senior appearances for (the hint),
// plus their total appearances across Spanish clubs.
function hintClubFor(footballerId: number): { club: string | null; clubWikiUrl: string | null; apps: number } {
  ...
  for (const s of stints) {
    if (!isSpanishClub(s.club)) continue
...
// [nationality, playerCount] for foreign countries (not Spain) with >= MIN
// players who have a senior stint at an Spanish club.
...
// All foreign players of a nationality who played for an Spanish club, with the
// hint club (most Spanish appearances).
function getCountryPlayers(nationality: string): LaLigaPlayer[] {
  const rows = sqlite
    .prepare(
  ...
  for (const r of rows) {
    if (!isSpanishClub(r.club)) continue
...
// ── Admin ───────────────────────────────────────────────────────────────────
// GET /api/la-liga/admin/countries
laLigaRouter.get('/admin/countries', (c) => {
  const valid = findValidCountries().sort((a, b) => b[1] - a[1])
  const enabled = sqlite
    .prepare(`SELECT nationality, round_size FROM la_liga_enabled_countries`)
...
// GET /api/la-liga/admin/countries/:nationality/players
laLigaRouter.get('/admin/countries/:nationality/players', (c) => {
  return c.json(getCountryPlayers(c.req.param('nationality')))
})

// POST /api/la-liga/admin/countries/enable { nationality, roundSize }
laLigaRouter.post(
...
// DELETE /api/la-liga/admin/countries/enable { nationality }
laLigaRouter.delete(
  '/admin/countries/enable',
  zValidator('json', z.object({ nationality: z.string().min(1) })),
  (c) => {
    sqlite
      .prepare(`DELETE FROM la_liga_enabled_countries WHERE LOWER(nationality) = LOWER(?)`)
...
// ── Schedule ────────────────────────────────────────────────────────────────
laLigaRouter.get('/schedule', (c) => {
  const rows = sqlite
    .prepare(`SELECT id, date, nationality, created_at FROM la_liga_schedule ORDER BY date ASC`)
    .all()
  return c.json(rows)
})

// GET /api/la-liga/schedule/rounds — playable rounds
laLigaRouter.get('/schedule/rounds', (c) => {
  const scheduled = sqlite
    .prepare(`SELECT date, nationality FROM la_liga_schedule ORDER BY date ASC`)
...
laLigaRouter.put(
  ...
  (c) => {
    ...
    const existing = sqlite.prepare(`SELECT id FROM la_liga_schedule WHERE date = ?`).get(date)
    if (existing) {
      sqlite.prepare(`UPDATE la_liga_schedule SET nationality = ? WHERE date = ?`).run(nationality, date)
    } else {
      sqlite.prepare(`INSERT INTO la_liga_schedule (date, nationality) VALUES (?, ?)`).run(date, nationality)
    }
    return c.json({ ok: true })
  },
)

laLigaRouter.delete('/schedule/:date', (c) => {
  sqlite.prepare(`DELETE FROM la_liga_schedule WHERE date = ?`).run(c.req.param('date'))
  return c.json({ ok: true })
})

laLigaRouter.delete('/schedule', (c) => {
  sqlite.prepare(`DELETE FROM la_liga_schedule`).run()
  return c.json({ ok: true })
})

// GET /api/la-liga/answers?nationality=X
laLigaRouter.get('/answers', (c) => {
...
// POST /api/la-liga/verify — a guess qualifies when the footballer's
// nationality matches the country AND they played for an Spanish club.
laLigaRouter.post(
  '/verify',
  ...
  async (c) => {
    ...
    const qualifies = (stintClubs: string[], nat: string | null | undefined) =>
      nationalitiesMatch(nat, nationality) && stintClubs.some(isSpanishClub)
    ...
    function invalidJson(name: string, nat: string | null | undefined, stintClubs: string[]) {
      ...
      const clubOk = stintClubs.some(isSpanishClub)
