...
import { scrapeWikipedia, createFromScrape, ApiError, type ScrapeResult } from '@/api/footballers'
...
export function AddFootballerPage() {
  ...
  async function handleBulkImport() {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        const result = await scrapeWikipedia(items[i].url)
        await createFromScrape({ ...result, stints: result.stints.map((s, si) => ({ ...s, sort_order: si })) })
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl">
      ...
      <h1 className="text-xl font-semibold mb-4">Add footballer</h1>
      ...
      {mode === 'single' && (
        <>
          <p className="text-sm text-muted-foreground mb-4">Paste a Wikipedia URL to scrape and review career data.</p>
              ...
              <div className="border rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input value={preview.nationality ?? ''} onChange={(e) => updateField('nationality', e.target.value)} placeholder="e.g. Italian" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground block mb-1">Position</label>
                    <Input value={preview.position ?? ''} onChange={(e) => updateField('position', e.target.value)} placeholder="e.g. Forward" />
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground block mb-1">Born</label>
                    <Input value={preview.born ?? ''} onChange={(e) => updateField('born', e.target.value)} placeholder="e.g. 1976-09-22" />
                  </div>
                </div>
              </div>
      ...
      {mode === 'bulk' && (
        <>
          <p className="text-sm text-muted-foreground mb-4">
            Paste one Wikipedia URL per line. Each footballer will be scraped and saved automatically.
          </p>
          ...
          {bulkItems.length === 0 ? (
            <>
              <textarea
                ...
                placeholder={'https://en.wikipedia.org/wiki/Roberto_Baggio\nhttps://en.wikipedia.org/wiki/Marco_van_Basten\n…'}
