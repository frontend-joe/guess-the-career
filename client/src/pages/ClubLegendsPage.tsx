  ...
  return (
    ...
    >
      ...
      {player ? (
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {hint.nationality && (
            <NationalityFlag nationality={hint.nationality} size={14} />
          )}
          {hint.position && <PositionBadge position={hint.position} />}
          <div className="h-px bg-gray-200 flex-1 rounded-full" />
          {hint.apps != null && (
            <span className="text-xs tabular-nums shrink-0">
              <span className="font-semibold text-gray-700">{hint.apps} apps</span>
...
interface RoundState {
  round: ClubLegendsScheduleRound;
  players: Player[] | null;
  guessedIds: Set<number>;
  wrongGuesses: Set<string>;
}
...
async function verifyGuess(
  ...
  try {
    ...
    const res = await fetch("/api/club-legends/verify", {
      method: "POST",
...
export function ClubLegendsPage() {
  ...
  const { requiredToPass } = useSettings("club_legends");
  ...
  useEffect(() => {
    ...
    fetch(`/api/club-legends/answers?club=${encodeURIComponent(currentRound.club)}`)
  ...
  const filteredProgressData = progressRounds
    ...
    .filter(({ i }) => {
      ...
      return rounds[i].club.toLowerCase().includes(term);
    });
  ...
  // position hint drawn from the top still-unguessed players.
    ...
    <div className="h-dvh flex flex-col w-full max-w-100 mx-auto font-sans">
      ...
      <div className="bg-[#0b0c1a] divide-soft-b relative flex items-center justify-between px-3 py-2.5 shrink-0">
        ...
        <div className="flex items-center gap-1">
          <GameSettingsButton gameKey="club_legends" />
          <button
