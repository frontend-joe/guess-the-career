...
import {
  ...
  setCountryEnabled,
  type AdminCountry,
  type InternationalLegendPlayer,
} from '@/api/international-legends-admin'

const PAGE_SIZE = 25

export function InternationalLegendsAdminPage() {
  ...
  const [playersByCountry, setPlayersByCountry] = useState<Record<string, InternationalLegendPlayer[]>>({})
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold">International Legends</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {loading ? 'Loading…' : `${total} qualifying countries · ${enabledCount} enabled`}
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => navigate('/admin/international-legends/schedule')}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-32 text-right">Legends (50+ caps)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                          ...
                          <ul className="divide-y">
                            {(playersByCountry[country.country] ?? []).map(p => (
                              <li key={p.id} className="flex items-center gap-3 px-4 py-2">
                                ...
                                <span className="text-xs text-muted-foreground">caps</span>
                              </li>
                            ))}
                          </ul>
                        )}
