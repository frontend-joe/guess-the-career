...
import {
  getSerieAScheduleRounds,
  type SerieAScheduleRound,
...
// ─── localStorage ───────────────────────────────────────────────────────────
const PROGRESS_KEY = "serie_a_progress";
...
interface RoundState { round: SerieAScheduleRound; players: Player[] | null; guessedIds: Set<number>; wrongGuesses: Set<string> }
...
async function verifyGuess(footballerName: string, footballerId: number | null, nationality: string): Promise<VerifyResult> {
  try {
    ...
    const res = await fetch("/api/serie-a/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!res.ok) return { valid: false, footballer: null, imported: false };
    return await res.json();
  } catch { return { valid: false, footballer: null, imported: false } }
}

export function SerieAPage() {
  const { compact } = useCompactMode();
  const { requiredToPass } = useSettings("serie_a");
  const [searchParams, setSearchParams] = useSearchParams();
  const [rounds, setRounds] = useState<SerieAScheduleRound[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/serie-a/answers?nationality=${encodeURIComponent(currentRound.nationality)}`)
  ...
  return (
    <div className="h-dvh flex flex-col w-full max-w-100 mx-auto font-sans">
      <div className="bg-[#0b0c1a] divide-soft-b relative flex items-center justify-between px-3 py-2.5 shrink-0">
        <GameMenu />
        <span className="absolute inset-0 flex items-center justify-center pointer-events-none text-white font-display text-sm tracking-wide uppercase">Serie A Players</span>
        <div className="flex items-center gap-1">
          <GameSettingsButton gameKey="serie_a" />
          ...
          <div className="flex-1 overflow-y-auto min-h-0 bg-gray-50 flex flex-col">
            {currentRound && (
              <GameHeader
                ...
                subtitle={`${currentRound.nationality} players who played in Serie A`}
              />
            )}
            {currentRound && (
