...
import {
  ...
  setClubEnabled,
  type AdminClub,
  type ClubMarksmanPlayer,
} from '@/api/club-marksman-admin'

const PAGE_SIZE = 25

export function ClubMarksmanAdminPage() {
  ...
  const [playersByClub, setPlayersByClub] = useState<Record<string, ClubMarksmanPlayer[]>>({})
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold">Club Marksman</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {loading ? 'Loading…' : `${total} qualifying clubs · ${enabledCount} enabled`}
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => navigate('/admin/club-marksman/schedule')}>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-32 text-right">Marksmen (50+ goals)</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {clubs.map(club => (
