...
internationalLegendsRouter.post(
  ...
  async (c) => {
    ...
    // A guess is valid when the player made >= MIN_APPS senior international
    // appearances for this country. Players not in our DB are scraped from
    // Wikipedia (and imported when they qualify) so ANY qualifying player counts,
    // not just the top few.
    const qualifies = (stints: { club: string; apps: number | null }[]) =>
    ...
    const verified = (id: number, fb: { name: string; photo_url: string | null }): LegendPlayer =>
      getCountryLegends(country).find((p) => p.id === id) ?? {
        id, name: fb.name, photo_url: fb.photo_url, apps: 0, position: null,
        hintClub: null, clubWikiUrl: null, years: null,
      };

    const invalidJson = (name: string, stints: { club: string; apps: number | null }[]) => {
      const reason = playedForCountry(stints, country) ? "not_top" : "wrong_nation";
      return { valid: false as const, foundName: name, capsForCountry: sumIntlApps(stints, country), imported: false, reason };
    };
