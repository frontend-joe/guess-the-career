...
const POSITION_PATTERNS: Record<string, string[]> = {
  ...
  right_back: ['%right back%', '%right-back%', '%right wing back%', '%right wingback%'],
  left_back: ['%left back%', '%left-back%', '%left wing back%', '%left wingback%'],
  centre_back: ['%centre back%', '%center back%', '%centre-back%', '%center-back%', '%central defender%', '%sweeper%'],
  ...
  forward: ['%forward%', '%centre forward%', '%center forward%'],
  striker: ['%striker%', '%centre forward%', '%center forward%'],
}

const VALID_NATIONS = new Set(Object.keys(NATION_NATIONALITIES))
const VALID_POSITIONS = new Set(Object.keys(POSITION_PATTERNS))

function matchesPosition(dbPosition: string | null, allPositions: string | null, positionKey: string): boolean {
  ...
  return check(dbPosition) || check(allPositions)
}
...
positionKnowledgeRouter.get('/players', async (c) => {
  ...
  if (!VALID_NATIONS.has(nation)) return c.json({ error: 'Invalid nation' }, 400)
  if (!VALID_POSITIONS.has(position)) return c.json({ error: 'Invalid position' }, 400)
  ...
  const posConditions = posPatterns.map(() => `LOWER(position) LIKE ? OR LOWER(all_positions) LIKE ?`).join(' OR ')

  const posParams = posPatterns.flatMap(p => [p, p])

  const rows = sqlite.prepare(`
    SELECT id, name, photo_url
    FROM footballers
    WHERE LOWER(nationality) IN (${natPlaceholders})
      AND (position IS NOT NULL OR all_positions IS NOT NULL)
      AND (${posConditions})
    ORDER BY name ASC
  `).all(...natLower, ...posParams) as { id: number; name: string; photo_url: string | null }[]
...
positionKnowledgeRouter.post(
  ...
  async (c) => {
    ...
    type FootballerRow = { id: number; name: string; nationality: string | null; position: string | null; all_positions: string | null; wikipedia_url: string; photo_url: string | null }
    ...
    if (playerId != null) {
      footballer = await db.select({
        id: footballers.id, name: footballers.name,
        nationality: footballers.nationality, position: footballers.position,
        all_positions: footballers.all_positions,
        wikipedia_url: footballers.wikipedia_url, photo_url: footballers.photo_url,
      }).from(footballers).where(eq(footballers.id, playerId)).limit(1).then(r => r[0])
    }
    ...
    if (!footballer) {
      ...
      footballer = await db.select({
        ...
        .where(sql`LOWER(normalize(${footballers.name})) = LOWER(normalize(${playerName}))`)
        .limit(1).then(r => r[0])
    ...
    if (footballer) {
      if (matchesNation(footballer.nationality, nation) && matchesPosition(footballer.position, footballer.all_positions, position)) {
      ...
      if (footballer.wikipedia_url && !matchesPosition(footballer.position, footballer.all_positions, position)) {
        try {
          const scraped = await scrapeWikipedia(footballer.wikipedia_url)
          await db.update(footballers)
            .set({ position: scraped.position, all_positions: scraped.all_positions ?? null, nationality: scraped.nationality ?? footballer.nationality, updated_at: sql`(datetime('now'))` })
            .where(eq(footballers.id, footballer.id))
          const n = scraped.nationality ?? footballer.nationality
          if (matchesNation(n, nation) && matchesPosition(scraped.position, scraped.all_positions, position)) {
    ...
    try {
      ...
      const byUrl = await db.select({
        id: footballers.id, name: footballers.name,
        nationality: footballers.nationality, position: footballers.position,
        all_positions: footballers.all_positions, photo_url: footballers.photo_url,
      }).from(footballers).where(eq(footballers.wikipedia_url, wikiUrl)).limit(1).then(r => r[0])

      if (byUrl) {
        if (matchesNation(byUrl.nationality, nation) && matchesPosition(byUrl.position, byUrl.all_positions, position)) {
      ...
      if (!matchesNation(scraped.nationality, nation) || !matchesPosition(scraped.position, scraped.all_positions, position)) {
        return c.json({ valid: false, footballer: null, imported: false })
      }
