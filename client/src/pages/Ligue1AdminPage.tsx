...
import {
  ...
  type Ligue1Country,
  type Ligue1Player,
} from '@/api/ligue-1-admin'

export function Ligue1AdminPage() {
  ...
  const [players, setPlayers] = useState<Record<string, Ligue1Player[]>>({})
  ...
  function patch(nationality: string, changes: Partial<Ligue1Country>) {
    setCountries((prev) => prev.map((c) => (c.nationality === nationality ? { ...c, ...changes } : c)))
  }

  async function toggle(c: Ligue1Country, enabled: boolean) {
  ...
  async function changeSize(c: Ligue1Country, roundSize: number) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl">
      <div className="flex items-start justify-between mb-6 gap-2 flex-wrap">
        <div>
          <h1 className="text-xl font-semibold">Ligue 1</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {loading ? 'Loading…' : `${countries.length} countries · ${enabledCount} in schedule`}
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => navigate('/admin/ligue-1/schedule')}>
