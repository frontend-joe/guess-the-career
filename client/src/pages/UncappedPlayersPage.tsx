...
import {
  getUncappedScheduleRounds,
  type UncappedScheduleRound,
...
// ─── localStorage ───────────────────────────────────────────────────────────
const PROGRESS_KEY = "uncapped_progress";
...
interface RoundState { round: UncappedScheduleRound; players: Player[] | null; guessedIds: Set<number>; wrongGuesses: Set<string> }
...
interface VerifyResult {
  ...
  imported: boolean;
  reason?: "wrong_nationality" | "capped" | "no_nationality";
}
...
async function verifyGuess(footballerName: string, footballerId: number | null, nationality: string): Promise<VerifyResult> {
  try {
    ...
    const res = await fetch("/api/uncapped-players/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!res.ok) return { valid: false, footballer: null, imported: false };
    return await res.json();
  } catch { return { valid: false, footballer: null, imported: false } }
}

export function UncappedPlayersPage() {
  const { compact } = useCompactMode();
  const { requiredToPass } = useSettings("uncapped_players");
  const [searchParams, setSearchParams] = useSearchParams();
  const [rounds, setRounds] = useState<UncappedScheduleRound[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/uncapped-players/answers?nationality=${encodeURIComponent(currentRound.nationality)}`)
  ...
  const progressRounds: ProgressRound[] = rounds.map((r, i) => {
    ...
    const target = requiredToPass(roundTarget(r));
    return {
  ...
  const totalGuessed = rounds.filter((r) => (roundStates[r.nationality]?.guessedIds.size ?? 0) >= requiredToPass(roundTarget(r))).length;
  ...
  return (
    <div className="h-dvh flex flex-col w-full max-w-100 mx-auto font-sans">
      <div className="bg-[#0b0c1a] divide-soft-b relative flex items-center justify-between px-3 py-2.5 shrink-0">
        <GameMenu />
        <span className="absolute inset-0 flex items-center justify-center pointer-events-none text-white font-display text-sm tracking-wide uppercase">Uncapped Players</span>
        <div className="flex items-center gap-1">
          <GameSettingsButton gameKey="uncapped_players" />
        ...
        <>
          ...
          {currentRound && (
            <div className="bg-[#1a1a2e] shrink-0 px-3 pt-3 pb-4">
              ...
              {!isDone && (
                <div className="mb-3">
                  <GuessSearchInput autoScrape={false} inputRef={inputRef} disabled={verifying} getKey={(f) => f.id} getLabel={(f) => f.name} getStatus={guessStatus} onSelect={(name, item) => submitGuess(name, item?.id ?? null)} />
                </div>
              )}
