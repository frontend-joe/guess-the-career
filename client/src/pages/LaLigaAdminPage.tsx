...
import {
  ...
  type LaLigaCountry,
  type LaLigaPlayer,
} from '@/api/la-liga-admin'

export function LaLigaAdminPage() {
  ...
  const [players, setPlayers] = useState<Record<string, LaLigaPlayer[]>>({})
  ...
  function patch(nationality: string, changes: Partial<LaLigaCountry>) {
    setCountries((prev) => prev.map((c) => (c.nationality === nationality ? { ...c, ...changes } : c)))
  }

  async function toggle(c: LaLigaCountry, enabled: boolean) {
  ...
  async function changeSize(c: LaLigaCountry, roundSize: number) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl">
      <div className="flex items-start justify-between mb-6 gap-2 flex-wrap">
        <div>
          <h1 className="text-xl font-semibold">La Liga</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {loading ? 'Loading…' : `${countries.length} countries · ${enabledCount} in schedule`}
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => navigate('/admin/la-liga/schedule')}>
