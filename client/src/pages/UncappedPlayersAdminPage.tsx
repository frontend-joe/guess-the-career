...
import {
  ...
  getCountryPlayers,
  setCountryEnabled,
  type UncappedCountry,
  type UncappedPlayer,
} from '@/api/uncapped-players-admin'

export function UncappedPlayersAdminPage() {
  ...
  const [players, setPlayers] = useState<Record<string, UncappedPlayer[]>>({})
  ...
  function patch(nationality: string, changes: Partial<UncappedCountry>) {
    setCountries((prev) => prev.map((c) => (c.nationality === nationality ? { ...c, ...changes } : c)))
  }

  async function toggle(c: UncappedCountry, enabled: boolean) {
  ...
  async function changeSize(c: UncappedCountry, roundSize: number) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl">
      <div className="flex items-start justify-between mb-6 gap-2 flex-wrap">
        <div>
          <h1 className="text-xl font-semibold">Uncapped Players</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {loading ? 'Loading…' : `${countries.length} countries · ${enabledCount} in schedule`}
          </p>
        </div>
        <Button variant="outline" size="sm" onClick={() => navigate('/admin/uncapped-players/schedule')}>
