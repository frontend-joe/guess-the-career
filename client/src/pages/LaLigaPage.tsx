...
import {
  getLaLigaScheduleRounds,
  type LaLigaScheduleRound,
...
// ─── localStorage ───────────────────────────────────────────────────────────
const PROGRESS_KEY = "la_liga_progress";
...
interface RoundState { round: LaLigaScheduleRound; players: Player[] | null; guessedIds: Set<number>; wrongGuesses: Set<string> }
...
async function verifyGuess(footballerName: string, footballerId: number | null, nationality: string): Promise<VerifyResult> {
  try {
    ...
    const res = await fetch("/api/la-liga/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!res.ok) return { valid: false, footballer: null, imported: false };
    return await res.json();
  } catch { return { valid: false, footballer: null, imported: false } }
}

export function LaLigaPage() {
  const { compact } = useCompactMode();
  const { requiredToPass } = useSettings("la_liga");
  const [searchParams, setSearchParams] = useSearchParams();
  const [rounds, setRounds] = useState<LaLigaScheduleRound[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/la-liga/answers?nationality=${encodeURIComponent(currentRound.nationality)}`)
    ...
    try {
      ...
      if (result.valid && result.footballer) {
        ...
        const msg = natWrong && result.foundNationality ? `${displayName} is actually ${result.foundNationality}`
          : result.reason === "wrong_club" ? `${displayName} didn't play in Spain`
  ...
  return (
    <div className="h-dvh flex flex-col w-full max-w-100 mx-auto font-sans">
      <div className="bg-[#0b0c1a] divide-soft-b relative flex items-center justify-between px-3 py-2.5 shrink-0">
        <GameMenu />
        <span className="absolute inset-0 flex items-center justify-center pointer-events-none text-white font-display text-sm tracking-wide uppercase">La Liga Players</span>
        <div className="flex items-center gap-1">
          <GameSettingsButton gameKey="la_liga" />
          ...
          <div className="flex-1 overflow-y-auto min-h-0 bg-gray-50 flex flex-col">
            {currentRound && (
              <GameHeader
                ...
                subtitle={`${currentRound.nationality} players who played in Spain`}
              />
            )}
            {currentRound && (
