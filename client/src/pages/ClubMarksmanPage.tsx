...
import {
  getClubMarksmanScheduleRounds,
  type ClubMarksmanScheduleRound,
...
interface RoundState {
  round: ClubMarksmanScheduleRound;
  players: Player[] | null;
  guessedIds: Set<number>;
  wrongGuesses: Set<string>;
}
...
async function verifyGuess(
  ...
  try {
    ...
    const res = await fetch("/api/club-marksman/verify", {
      method: "POST",
...
export function ClubMarksmanPage() {
  ...
  const { requiredToPass } = useSettings("club_marksman");
  ...
  useEffect(() => {
    ...
    fetch(`/api/club-marksman/answers?club=${encodeURIComponent(currentRound.club)}`)
    ...
    <div className="h-dvh flex flex-col w-full max-w-100 mx-auto font-sans">
      ...
      <div className="bg-[#0b0c1a] divide-soft-b relative flex items-center justify-between px-3 py-2.5 shrink-0">
        ...
        <div className="flex items-center gap-1">
          <GameSettingsButton gameKey="club_marksman" />
          <button
          ...
          <div className="flex-1 overflow-y-auto min-h-0 bg-gray-50 flex flex-col">
            {currentRound && (
              <GameHeader
                ...
                subtitle="Players with 50+ goals"
                difficulty={
