...
import {
  ...
  resolvePlayer,
  resolvePlayerByUrl,
  type RecordSalesClubDetail,
} from '@/api/record-sales-admin'

export function RecordSalesDetailPage() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const [data, setData] = useState<RecordSalesClubDetail | null>(null)
  ...
  const reload = useCallback(async () => {
    if (!id) return
    try {
      setData(await getRecordSalesClubDetail(Number(id)))
      setError(null)
    } catch (e) {
  ...
  async function relinkClub(name: string) {
    if (!id) return
    try {
      await updateRecordSalesClub(Number(id), { club: name })
      await reload()
    } catch (e) {
  ...
  // Fix the club (who the player joined) if it matched the wrong club on
      ...
      {data && (
        <>
          <div className="flex items-center gap-4 text-sm mb-3 flex-wrap">
            <span className="text-muted-foreground">{data.signings.length} sales (ordered by fee, as in the game)</span>
            {unlinked > 0 ? (
            ...
            {data.signings.map((t) => (
              ...
              >
                ...
                <ClubPicker
                  ...
                  title="Wrong club? Click to change who the player joined"
