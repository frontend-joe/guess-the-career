...
function hintClubFor(footballerId: number): { club: string | null; clubWikiUrl: string | null; apps: number } {
  ...
  for (const s of stints) {
    if (!isItalianClub(s.club)) continue
...
// [nationality, playerCount] for foreign countries (not Italy) with >= MIN
// players who have a senior stint at an Italian club.
...
// All foreign players of a nationality who played for an Italian club, with the
// hint club (most Italian appearances).
...
// ── Admin ───────────────────────────────────────────────────────────────────
// GET /api/serie-a/admin/countries
serieARouter.get('/admin/countries', (c) => {
  const valid = findValidCountries().sort((a, b) => b[1] - a[1])
  const enabled = sqlite
    .prepare(`SELECT nationality, round_size FROM serie_a_enabled_countries`)
...
// GET /api/serie-a/admin/countries/:nationality/players
serieARouter.get('/admin/countries/:nationality/players', (c) => {
  return c.json(getCountryPlayers(c.req.param('nationality')))
})

// POST /api/serie-a/admin/countries/enable { nationality, roundSize }
serieARouter.post(
  ...
  (c) => {
    ...
    sqlite
      .prepare(
        `INSERT INTO serie_a_enabled_countries (nationality, round_size) VALUES (?, ?)
...
// DELETE /api/serie-a/admin/countries/enable { nationality }
serieARouter.delete(
  '/admin/countries/enable',
  zValidator('json', z.object({ nationality: z.string().min(1) })),
  (c) => {
    sqlite
      .prepare(`DELETE FROM serie_a_enabled_countries WHERE LOWER(nationality) = LOWER(?)`)
...
// ── Schedule ────────────────────────────────────────────────────────────────
serieARouter.get('/schedule', (c) => {
  const rows = sqlite
    .prepare(`SELECT id, date, nationality, created_at FROM serie_a_schedule ORDER BY date ASC`)
    .all()
  return c.json(rows)
})

// GET /api/serie-a/schedule/rounds — playable rounds
serieARouter.get('/schedule/rounds', (c) => {
  const scheduled = sqlite
    .prepare(`SELECT date, nationality FROM serie_a_schedule ORDER BY date ASC`)
...
serieARouter.put(
  ...
  (c) => {
    ...
    const existing = sqlite.prepare(`SELECT id FROM serie_a_schedule WHERE date = ?`).get(date)
    if (existing) {
      sqlite.prepare(`UPDATE serie_a_schedule SET nationality = ? WHERE date = ?`).run(nationality, date)
    } else {
      sqlite.prepare(`INSERT INTO serie_a_schedule (date, nationality) VALUES (?, ?)`).run(date, nationality)
    }
    return c.json({ ok: true })
  },
)

serieARouter.delete('/schedule/:date', (c) => {
  sqlite.prepare(`DELETE FROM serie_a_schedule WHERE date = ?`).run(c.req.param('date'))
  return c.json({ ok: true })
})

serieARouter.delete('/schedule', (c) => {
  sqlite.prepare(`DELETE FROM serie_a_schedule`).run()
  return c.json({ ok: true })
})

// GET /api/serie-a/answers?nationality=X
serieARouter.get('/answers', (c) => {
...
// POST /api/serie-a/verify — a guess qualifies when the footballer's
// nationality matches the country AND they played for an Italian club.
serieARouter.post(
  '/verify',
  ...
  async (c) => {
    ...
    const qualifies = (stintClubs: string[], nat: string | null | undefined) =>
      nationalitiesMatch(nat, nationality) && stintClubs.some(isItalianClub)
