...
function nationalitiesMatch(playerNat: string | null | undefined, targetNat: string): boolean {
...
nationalsRouter.get('/admin/combos', (c) => {
  ...
  if (existingCount === 0 && validCombos.length > 0) {
    const insert = sqlite.prepare(`INSERT OR IGNORE INTO nationals_enabled_combos (nationality, club) VALUES (?, ?)`)
...
nationalsRouter.post(
  '/admin/combos/enable',
  zValidator('json', z.object({ nationality: z.string().min(1), club: z.string().min(1) })),
  (c) => {
    const { nationality, club } = c.req.valid('json')
    sqlite.prepare(`INSERT OR IGNORE INTO nationals_enabled_combos (nationality, club) VALUES (?, ?)`).run(nationality, normalizeClubAlias(club))
...
nationalsRouter.delete(
  ...
  (c) => {
    const { nationality, club } = c.req.valid('json')
    sqlite.prepare(`DELETE FROM nationals_enabled_combos WHERE LOWER(nationality) = LOWER(?) AND LOWER(club) = LOWER(?)`).run(nationality, normalizeClubAlias(club))
    return c.json({ ok: true })
  }
)
...
nationalsRouter.get('/schedule', (c) => {
  ...
  ).all() as { id: number; date: string; nationality: string; club: string; created_at: string }[]
  return c.json(rows)
})
...
nationalsRouter.get('/schedule/rounds', (c) => {
  ...
  const rounds = scheduled.map(row => {
    const clubRow = sqlite.prepare(`SELECT wikipedia_url FROM clubs WHERE LOWER(name) = LOWER(?) LIMIT 1`).get(row.club) as { wikipedia_url: string | null } | undefined
    return {
      date: row.date,
...
nationalsRouter.put(
  ...
  (c) => {
    ...
    const { nationality, club } = c.req.valid('json')
    const existing = sqlite.prepare(`SELECT id FROM nationals_schedule WHERE date = ?`).get(date)
    if (existing) {
      sqlite.prepare(`UPDATE nationals_schedule SET nationality = ?, club = ? WHERE date = ?`).run(nationality, club, date)
    } else {
      sqlite.prepare(`INSERT INTO nationals_schedule (date, nationality, club) VALUES (?, ?, ?)`).run(date, nationality, club)
    }
    return c.json({ ok: true })
  }
)
...
nationalsRouter.get('/answers', (c) => {
  ...
  if (!nationality || !club) return c.json({ error: 'nationality and club required' }, 400)
  ...
  const rows = sqlite.prepare(`
    ...
    ORDER BY f.name ASC
  `).all(...variants, nationality) as { id: number; name: string; photo_url: string | null }[]
...
nationalsRouter.post(
  ...
  async (c) => {
    const { footballerName, footballerId, nationality, club } = c.req.valid('json')

    function checkQualifies(stintClubs: string[], nat: string | null | undefined): boolean {
    ...
    try {
      ...
      let firstResult = await wikiSearch(footballerName + ' ' + club)
      ...
      const byUrl = await db.select({ id: footballers.id, name: footballers.name, photo_url: footballers.photo_url, nationality: footballers.nationality })
