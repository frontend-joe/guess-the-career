...
import {
  getInternationalLegendsScheduleRounds,
  type InternationalLegendsScheduleRound,
...
const PROGRESS_KEY = "il_progress";
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {hint.apps != null && (
            <span className="text-xs tabular-nums shrink-0">
              <span className="font-semibold text-gray-700">{hint.apps} caps</span>
...
interface RoundState {
  round: InternationalLegendsScheduleRound;
  players: Player[] | null;
  guessedIds: Set<number>;
  wrongGuesses: Set<string>;
}
...
async function verifyGuess(
  ...
  try {
    ...
    const res = await fetch("/api/international-legends/verify", {
      method: "POST",
...
export function InternationalLegendsPage() {
  const { compact } = useCompactMode();
  const { requiredToPass } = useSettings("international_legends");
  // The pass goal — and the number of slots shown — scale with the setting.
  const passTarget = requiredToPass(ROUND_TARGET);
  const [searchParams, setSearchParams] = useSearchParams();
  const [rounds, setRounds] = useState<InternationalLegendsScheduleRound[]>([]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/international-legends/answers?country=${encodeURIComponent(currentRound.country)}`)
        ...
        if (result.valid && result.footballer) {
          ...
          const msg =
            result.reason === "not_top"
              ? `${displayName} only has ${result.capsForCountry ?? 0} caps for ${currentRound.country}`
    ...
    <div className="h-dvh flex flex-col w-full max-w-100 mx-auto font-sans">
      ...
      <div className="bg-[#0b0c1a] divide-soft-b flex items-center justify-between px-3 py-2.5 shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <GameSettingsButton gameKey="international_legends" />
        </div>
      </div>
