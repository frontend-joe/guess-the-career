...
import {
  record_sales_clubs,
  record_sales_players,
  record_sales_schedule,
  footballers,
} from '../db/schema.ts'
import { scrapeTransfermarktRecordSales } from '../services/scraper.ts'
...
export const recordSalesRouter = new Hono()
...
async function resolveMissingPlayers(rows: { rowId: number; name: string }[], clubHint: string) {
  ...
  for (const r of rows) {
    ...
    if (id) {
      await db
        .update(record_sales_players)
        .set({ footballer_id: id })
        .where(eq(record_sales_players.id, r.rowId))
    }
...
// POST /api/record-sales/scrape
recordSalesRouter.post(
  '/scrape',
  zValidator('json', z.object({ url: z.string().url() })),
  async (c) => {
    const { url } = c.req.valid('json')
    try {
      const result = await scrapeTransfermarktRecordSales(url)
...
// POST /api/record-sales — create/replace a club round + its signings,
// auto-importing any players not already in the DB (Wikipedia search + scrape).
recordSalesRouter.post('/', zValidator('json', importSchema), async (c) => {
  const body = c.req.valid('json')

  let [clubRow] = await db
    .select()
    .from(record_sales_clubs)
    .where(eq(record_sales_clubs.source_url, body.source_url))
    .limit(1)

  if (clubRow) {
    await db.delete(record_sales_players).where(eq(record_sales_players.club_id, clubRow.id))
    await db
      .update(record_sales_clubs)
      ...
      .where(eq(record_sales_clubs.id, clubRow.id))
  } else {
    ;[clubRow] = await db
      .insert(record_sales_clubs)
      .values({
        club: body.club,
  ...
  for (const s of body.signings) {
    ...
    const [row] = await db
      ...
      .returning({ id: record_sales_players.id })
...
// POST /api/record-sales/resolve-player — find or scrape+create a footballer.
recordSalesRouter.post(
  '/resolve-player',
...
// POST /api/record-sales/resolve-url — scrape+create a footballer from an
...
// ── Clubs admin ──────────────────────────────────────────────────────────────
// GET /api/record-sales/clubs
recordSalesRouter.get('/clubs', (c) => {
  const rows = sqlite
    .prepare(
      `SELECT rc.id, rc.club, rc.club_wikipedia_url, rc.source_url, rc.active, rc.created_at,
              (SELECT COUNT(*) FROM record_sales_players sp WHERE sp.club_id = rc.id) AS player_count,
              (SELECT COUNT(*) FROM record_sales_players sp WHERE sp.club_id = rc.id AND sp.footballer_id IS NULL) AS unlinked_count
       FROM record_sales_clubs rc
...
// GET /api/record-sales/clubs/:id — club + its signings in game order
recordSalesRouter.get('/clubs/:id', async (c) => {
  const id = parseInt(c.req.param('id'), 10)
  const [clubRow] = await db.select().from(record_sales_clubs).where(eq(record_sales_clubs.id, id)).limit(1)
  if (!clubRow) return c.json({ error: 'Not found' }, 404)
  return c.json({ club: clubRow, signings: clubSignings(id) })
})

// PATCH /api/record-sales/clubs/:id — toggle active / relink club name
recordSalesRouter.patch(
  '/clubs/:id',
  ...
  async (c) => {
    ...
    const [updated] = await db.update(record_sales_clubs).set(set).where(eq(record_sales_clubs.id, id)).returning()
    if (!updated) return c.json({ error: 'Not found' }, 404)
    return c.json(updated)
  },
)

// DELETE /api/record-sales/clubs/:id
recordSalesRouter.delete('/clubs/:id', async (c) => {
  const id = parseInt(c.req.param('id'), 10)
  await db.delete(record_sales_clubs).where(eq(record_sales_clubs.id, id))
  return c.json({ ok: true })
})

// PATCH /api/record-sales/signings/:id — fix from-club / link footballer
recordSalesRouter.patch(
  '/signings/:id',
  zValidator('json', z.object({
  ...
  async (c) => {
    ...
    if (Object.keys(set).length === 0) return c.json({ error: 'Nothing to update' }, 400)
    const [updated] = await db.update(record_sales_players).set(set).where(eq(record_sales_players.id, id)).returning()
    if (!updated) return c.json({ error: 'Not found' }, 404)
    return c.json({ ok: true })
  },
)

// POST /api/record-sales/clubs/:id/relink — resolve still-unlinked players
recordSalesRouter.post('/clubs/:id/relink', async (c) => {
  const id = parseInt(c.req.param('id'), 10)
  const [clubRow] = await db.select({ club: record_sales_clubs.club }).from(record_sales_clubs).where(eq(record_sales_clubs.id, id)).limit(1)
  const unlinked = sqlite
    .prepare(`SELECT id, player_name FROM record_sales_players WHERE club_id = ? AND footballer_id IS NULL`)
...
// ── Schedule ─────────────────────────────────────────────────────────────────
// GET /api/record-sales/schedule — admin list with club meta
recordSalesRouter.get('/schedule', async (c) => {
  const rows = await db
    .select({
      id: record_sales_schedule.id,
      date: record_sales_schedule.date,
      club_id: record_sales_schedule.club_id,
      club: record_sales_clubs.club,
    })
    .from(record_sales_schedule)
    .leftJoin(record_sales_clubs, eq(record_sales_schedule.club_id, record_sales_clubs.id))
    .orderBy(record_sales_schedule.date)
  return c.json(rows)
})

// GET /api/record-sales/schedule/rounds — playable rounds for the game
recordSalesRouter.get('/schedule/rounds', (c) => {
  const rows = sqlite
    .prepare(
      `SELECT rss.date, rss.club_id, rc.club, rc.club_wikipedia_url
       FROM record_sales_schedule rss
       JOIN record_sales_clubs rc ON rss.club_id = rc.id
...
// PUT /api/record-sales/schedule/:date — upsert assignment
recordSalesRouter.put(
  '/schedule/:date',
  zValidator('json', z.object({ club_id: z.number().int() })),
  async (c) => {
    const date = c.req.param('date')
    const { club_id } = c.req.valid('json')
    const existing = await db.select().from(record_sales_schedule).where(eq(record_sales_schedule.date, date))
    if (existing.length > 0) {
      const [updated] = await db.update(record_sales_schedule).set({ club_id }).where(eq(record_sales_schedule.date, date)).returning()
      return c.json(updated)
    }
    const [created] = await db.insert(record_sales_schedule).values({ date, club_id }).returning()
    return c.json(created, 201)
  },
)

// DELETE /api/record-sales/schedule — clear all
recordSalesRouter.delete('/schedule', async (c) => {
  await db.delete(record_sales_schedule)
  return c.json({ ok: true })
})

// DELETE /api/record-sales/schedule/:date
recordSalesRouter.delete('/schedule/:date', async (c) => {
  const date = c.req.param('date')
  const [deleted] = await db.delete(record_sales_schedule).where(eq(record_sales_schedule.date, date)).returning()
