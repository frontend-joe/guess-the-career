...
// Record Signings game (transfermarkt-scraped club record signings). A round =
// one club; record_signings_players = the club's top signings (by fee).
export const record_signings_clubs = sqliteTable('record_signings_clubs', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  club: text('club').notNull(),
  club_wikipedia_url: text('club_wikipedia_url'),
  source_url: text('source_url').notNull().unique(),
...
export const record_signings_players = sqliteTable('record_signings_players', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  club_id: integer('club_id').notNull().references(() => record_signings_clubs.id, { onDelete: 'cascade' }),
  footballer_id: integer('footballer_id').references(() => footballers.id, { onDelete: 'set null' }),
  player_name: text('player_name').notNull(),
  nationality: text('nationality'),
  position: text('position'),
...
export const record_signings_schedule = sqliteTable('record_signings_schedule', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  date: text('date').notNull().unique(),
  club_id: integer('club_id').references(() => record_signings_clubs.id, { onDelete: 'set null' }),
  created_at: text('created_at').notNull().default(sql`(datetime('now'))`),
})
