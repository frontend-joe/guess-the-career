...
import {
  getRecordSalesRounds,
  type RecordSalesRound,
  type RoundSigning,
...
const PROGRESS_KEY = "record_sales_schedule_progress";
...
function roundKey(clubId: number): string {
...
function buildRounds(data: RecordSalesRound[], saved: SavedProgress): RoundResult[] {
  return data.map((r) => {
    const prog = saved[roundKey(r.clubId)];
    return {
      date: r.date,
      clubId: r.clubId,
      club: r.club,
      ...
      // Clamp saved indices to the current signing list so old progress
      // doesn't inflate counts if the list size changes.
      guessedIndices: prog ? new Set(prog.guessedIndices.filter((i) => i < r.signings.length)) : new Set<number>(),
...
export function RecordSalesPage() {
  ...
  function persist(round: RoundResult, guessed: Set<number>, wrong: Set<string>, state: RoundState) {
    ...
    progress[roundKey(round.clubId)] = {
    ...
    <div className="h-dvh flex flex-col w-full max-w-[400px] mx-auto font-sans">
      ...
      <div className="bg-[#0b0c1a] divide-soft-b relative flex items-center justify-between px-3 py-2.5 shrink-0">
        ...
        <div className="flex items-center gap-1">
          <GameSettingsButton gameKey="record_sales" />
          {rounds.length > 0 ? (
            showProgress ? (
              ...
              <div className="flex-1 overflow-y-auto">
                <OverallProgressScreen
                  ...
                  rounds={filtered.map(({ r }) => ({
                    name: r.club,
                    subtitle: `${r.signings.length} sales`,
        ...
        {!loading && !error && !showFinalScore && !showProgress && currentRound && (
          <>
            ...
            <div className="rounded-xl overflow-hidden border border-gray-200 bg-white">
              {currentRound.signings.slice(0, roundTotal).map((t, i) => {
                ...
                return (
                  <div key={t.id} className="flex items-center gap-2 px-2.5 py-2 border-b border-gray-100 last:border-0">
                    ...
                    {/* From club (persistent hint), right-aligned by the fee */}
          ...
          {currentRound && !isRoundDone && (
            <div className="mb-3">
              <GuessSearchInput autoScrape={false}
                key={roundKey(currentRound.clubId)}
                inputRef={inputRef}
                getKey={(f) => f.id}
                getLabel={(f) => f.name}
  ...
  return (
    <div className="flex flex-col items-center px-4 py-8 gap-6">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm mt-2">{totalGuessed} / {totalPlayers} sales</p>
      </div>
        ...
        {rounds.map((r, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3 bg-white rounded-xl px-3 py-2.5 border border-gray-100">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-gray-900 truncate">{r.club}</p>
