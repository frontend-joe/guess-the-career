...
export const ligue1Router = new Hono()

const MIN_PLAYERS = 3


// The French club a player made the most senior appearances for (the hint),
// plus their total appearances across French clubs.
function hintClubFor(footballerId: number): { club: string | null; clubWikiUrl: string | null; apps: number } {
  ...
  for (const s of stints) {
    if (!isFrenchClub(s.club)) continue
...
// [nationality, playerCount] for foreign countries (not France) with >= MIN
// players who have a senior stint at an French club.
...
// All foreign players of a nationality who played for an French club, with the
// hint club (most French appearances).
function getCountryPlayers(nationality: string): Ligue1Player[] {
  const rows = sqlite
    .prepare(
  ...
  for (const r of rows) {
    if (!isFrenchClub(r.club)) continue
...
// ── Admin ───────────────────────────────────────────────────────────────────
// GET /api/ligue-1/admin/countries
ligue1Router.get('/admin/countries', (c) => {
  const valid = findValidCountries().sort((a, b) => b[1] - a[1])
  const enabled = sqlite
    .prepare(`SELECT nationality, round_size FROM ligue1_enabled_countries`)
...
// GET /api/ligue-1/admin/countries/:nationality/players
ligue1Router.get('/admin/countries/:nationality/players', (c) => {
  return c.json(getCountryPlayers(c.req.param('nationality')))
})

// POST /api/ligue-1/admin/countries/enable { nationality, roundSize }
ligue1Router.post(
  ...
  (c) => {
    ...
    sqlite
      .prepare(
        `INSERT INTO ligue1_enabled_countries (nationality, round_size) VALUES (?, ?)
...
// DELETE /api/ligue-1/admin/countries/enable { nationality }
ligue1Router.delete(
  '/admin/countries/enable',
  zValidator('json', z.object({ nationality: z.string().min(1) })),
  (c) => {
    sqlite
      .prepare(`DELETE FROM ligue1_enabled_countries WHERE LOWER(nationality) = LOWER(?)`)
...
// ── Schedule ────────────────────────────────────────────────────────────────
ligue1Router.get('/schedule', (c) => {
  const rows = sqlite
    .prepare(`SELECT id, date, nationality, created_at FROM ligue1_schedule ORDER BY date ASC`)
    .all()
  return c.json(rows)
})

// GET /api/ligue-1/schedule/rounds — playable rounds
ligue1Router.get('/schedule/rounds', (c) => {
  const scheduled = sqlite
    .prepare(`SELECT date, nationality FROM ligue1_schedule ORDER BY date ASC`)
...
ligue1Router.put(
  ...
  (c) => {
    ...
    const existing = sqlite.prepare(`SELECT id FROM ligue1_schedule WHERE date = ?`).get(date)
    if (existing) {
      sqlite.prepare(`UPDATE ligue1_schedule SET nationality = ? WHERE date = ?`).run(nationality, date)
    } else {
      sqlite.prepare(`INSERT INTO ligue1_schedule (date, nationality) VALUES (?, ?)`).run(date, nationality)
    }
    return c.json({ ok: true })
  },
)

ligue1Router.delete('/schedule/:date', (c) => {
  sqlite.prepare(`DELETE FROM ligue1_schedule WHERE date = ?`).run(c.req.param('date'))
  return c.json({ ok: true })
})

ligue1Router.delete('/schedule', (c) => {
  sqlite.prepare(`DELETE FROM ligue1_schedule`).run()
  return c.json({ ok: true })
})

// GET /api/ligue-1/answers?nationality=X
ligue1Router.get('/answers', (c) => {
...
// POST /api/ligue-1/verify — a guess qualifies when the footballer's
// nationality matches the country AND they played for an French club.
ligue1Router.post(
  '/verify',
  ...
  async (c) => {
    ...
    const qualifies = (stintClubs: string[], nat: string | null | undefined) =>
      nationalitiesMatch(nat, nationality) && stintClubs.some(isFrenchClub)
    ...
    function invalidJson(name: string, nat: string | null | undefined, stintClubs: string[]) {
      ...
      const clubOk = stintClubs.some(isFrenchClub)
